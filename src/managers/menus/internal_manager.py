...
# src/managers/internal_manager.py
...
class InternalManager(BaseManager):
    def __init__(self, display_manager, volumio_config, mode_manager, window_size=3, y_offset=0, line_spacing=16):
        ...
        self.mode_name = "internal"
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.setLevel(logging.INFO)
        self.logger.info("InternalManager initialized.")
        ...
        self.default_start_uri = "music-library/INTERNAL"  # Default starting URI
    ...
    def handle_mode_change(self, current_mode):
        """Handle mode changes."""
        self.logger.info(f"InternalManager handling mode change to: {current_mode}")
        if current_mode == "internal":
            self.logger.info("Entering internal mode.")
            self.start_mode(start_uri="music-library/INTERNAL")  # Start directly in "Music" folder
        elif current_mode == "usbinternal":
            self.logger.info("Entering USB internal mode.")
            self.start_mode(start_uri="music-internal/USB")
        elif self.is_active:
            self.logger.info("Exiting internal mode.")
            self.stop_mode()

    def start_mode(self, start_uri=None):
        if self.is_active:
            self.logger.debug("InternalManager: internal mode already active.")
            return

        self.logger.info("InternalManager: Starting internal mode.")
        self.is_active = True
        ...
        # Use the provided start_uri or default to "music-internal"
        self.current_path = start_uri if start_uri else self.default_start_uri
        self.logger.info(f"InternalManager: Starting navigation at URI: {self.current_path}")

        self.display_loading_screen()
        self.fetch_navigation(self.current_path)

        # Log the reset state
        self.logger.debug(f"InternalManager: Mode active: {self.is_active}, Selection Index: {self.current_selection_index}, Window Start Index: {self.window_start_index}")

        # Start timeout timer (e.g. 5 seconds)
        self.timeout_timer = threading.Timer(3.0, self.internal_timeout)
        self.timeout_timer.start()

    def internal_timeout(self):
        """Called when internal navigation has not loaded within the timeout period."""
        if not self.current_menu_items:
            self.logger.warning("InternalManager: Timeout reached, no navigation data received.")

            def draw(draw_obj):
                draw_obj.text(
                    (10, self.y_offset),
                    "Your internal is not loading...",
    ...
    def stop_mode(self):
        if not self.is_active:
            self.logger.debug("InternalManager: internal mode already inactive.")
            return
        self.is_active = False
        self.display_manager.clear_screen()
        self.logger.info("InternalManager: Stopped internal mode and cleared display.")

    def fetch_navigation(self, uri):
        """Fetch navigation data dynamically for any folder in the music internal."""
        self.logger.info(f"InternalManager: Fetching navigation data for URI: {uri}")
        try:
            # API request to fetch navigation data
            response = self.session.get(f"{self.base_url}/api/v1/browse?uri={quote(uri)}")
            self.logger.debug(f"InternalManager: Received Status Code: {response.status_code}")
            self.logger.debug(f"InternalManager: Received Response: {response.text}")

            # Handle non-200 status codes
            if response.status_code != 200:
                self.logger.error(f"InternalManager: Failed to fetch data. Status Code: {response.status_code}")
            ...
            if not lists:
                self.logger.warning("InternalManager: No lists available in the navigation data.")
            ...
            if not items:
                self.logger.info("InternalManager: No items in the current folder.")
                self.display_no_items()
                return
            ...
            self.logger.info(f"InternalManager: Fetched {len(self.current_menu_items)} items for URI: {uri}")
            if self.is_active:
                self.display_menu()

        except ValueError as ve:
            self.logger.error(f"InternalManager: JSON decoding failed: {ve}")
            self.display_error_message("Fetch Error", f"Invalid response format: {ve}")
        except Exception as e:
            self.logger.error(f"InternalManager: Exception occurred while fetching navigation: {str(e)}")
    ...
    def select_item(self):
        ...
        if not self.is_active or not self.current_menu_items:
            self.logger.warning("InternalManager: Select attempted while inactive or no items available.")
            return

        selected_item = self.current_menu_items[self.current_selection_index]
        self.logger.info(f"InternalManager: Selected item: {selected_item}")
        ...
        if 'action' in selected_item:
            ...
            if item_type in ["folder", "internal-folder", "streaming-category", "streaming-folder", "remdisk"]:
                # Check if the selected item is an album
                if self.is_album_folder(selected_item):
                    # Display album options (submenu)
                    self.logger.info(f"InternalManager: Displaying options for album: {selected_item.get('title')}")
                    self.display_folder_or_album_options(selected_item)
                else:
                    # Navigate into the folder
                    self.logger.info(f"InternalManager: Navigating into: {selected_item.get('title')}")
                ...
                self.logger.info(f"InternalManager: Playing song: {selected_item.get('title')}")
                self.replace_and_play(selected_item)

            elif item_type == "webradio":
                # Handle web radio streams if necessary
                self.logger.info(f"InternalManager: Playing web radio: {selected_item.get('title')}")
                self.replace_and_play(selected_item)

            else:
                self.logger.warning(f"InternalManager: Unknown item type '{item_type}'.")
    ...
    def is_album_folder(self, item):
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.warning(f"InternalManager: Failed to fetch contents for album check: {folder_uri}")
                return False
            ...
            self.logger.error(f"InternalManager: Exception during album check: {e}")
            return False

    def display_folder_or_album_options(self, folder_item):
        """Display options for a selected album as a new menu."""
        self.logger.info(f"InternalManager: Displaying options for album: {folder_item.get('title')}")
    ...
    def perform_action(self, action, data):
        ...
        if action == "play_album":
            self.logger.info(f"InternalManager: Playing entire album: {data.get('title')}")
            self.play_album_or_folder(data)
        elif action == "select_songs":
            self.logger.info(f"InternalManager: Fetching songs for album: {data.get('title')}")
            self.menu_stack.append(self.current_path)  # Save current path
            self.current_path = data.get("uri")
            self.display_loading_screen()
            self.fetch_navigation(self.current_path)
        elif action == "back":
            self.logger.info("InternalManager: Going back to the previous menu.")
            self.pop_menu()
        else:
            self.logger.warning(f"InternalManager: Unknown action '{action}'.")
    ...
    def play_album_or_folder(self, folder_item):
        ...
        if not folder_uri:
            self.logger.error("InternalManager: Selected folder does not have a valid URI.")
            self.display_error_message("Playback Error", "Selected folder cannot be played.")
            return

        self.logger.info(f"InternalManager: Playing all songs from folder: {folder_item.get('title')}")
    ...
    def _play_album_or_folder_thread(self, folder_uri, album_title):
        ...
        try:
            ...
            if replace_response.status_code == 200:
                self.logger.info(f"InternalManager: Playback started successfully for the album: {album_title}")
                self.display_success_message("Playback Started", f"Playing album: {album_title}")
            else:
                self.logger.error(f"InternalManager: Failed to start playback for album: {album_title}. Status Code: {replace_response.status_code}, Response: {replace_response.text}")
                self.display_error_message("Playback Error", f"Failed to start playback: {replace_response.status_code}")

        except Exception as e:
            self.logger.error(f"InternalManager: Failed to play album: {str(e)}")
    ...
    def replace_and_play(self, item):
        ...
        if not song_uri:
            self.logger.error("InternalManager: Selected item does not have a valid URI.")
            self.display_error_message("Playback Error", "Selected item cannot be played.")
            return

        self.logger.info(f"InternalManager: Replacing and playing item: {item.get('title')}")
        try:
            replace_and_play_url = f"{self.base_url}/api/v1/replaceAndPlay"
            data = {
            ...
            if response.status_code == 200:
                self.logger.info(f"InternalManager: Playback started successfully for: {item.get('title')}")
                self.display_success_message("Playback Started", f"Playing: {item.get('title')}")
            else:
                self.logger.error(f"InternalManager: Failed to start playback. Status Code: {response.status_code}, Response: {response.text}")
                self.display_error_message("Playback Error", f"Failed to start playback: {response.status_code}")

        except Exception as e:
            self.logger.error(f"InternalManager: Exception occurred while trying to play item: {str(e)}")
            self.display_error_message("Playback Error", f"Could not play item: {str(e)}")

    def display_loading_screen(self):
        """Show a loading screen."""
        self.logger.info("InternalManager: Displaying loading screen.")
    ...
    def pop_menu(self):
        ...
        if not self.menu_stack:
            self.logger.warning("InternalManager: Menu stack is empty, cannot pop.")
            return
        ...
        self.logger.info(f"InternalManager: Returning to menu: {menu_title}")
    ...
    def scroll_selection(self, direction):
        ...
        if not self.is_active or not self.current_menu_items:
            self.logger.warning("InternalManager: Scroll attempted with no active items.")
            return
        ...
        self.logger.debug(f"InternalManager: Scrolled from {previous_index} to {self.current_selection_index}.")
        self.display_menu()

    def display_no_items(self):
        """Display a message if no items are available."""
        self.logger.info("InternalManager: No items available.")
    ...
    def display_error_message(self, title, message):
        ...
        self.logger.info(f"InternalManager: Displaying error message: {title} - {message}")
    ...
    def display_success_message(self, title, message):
        ...
        self.logger.info(f"InternalManager: Displaying success message: {title} - {message}")
    ...
    def go_back(self):
        ...
        if self.menu_stack:
            ...
            self.logger.info("InternalManager: Already at root level, cannot go back.")
    ...
    def update_song_info(self, state):
        ...
        self.logger.info("InternalManager: Updating playback metrics display.")
